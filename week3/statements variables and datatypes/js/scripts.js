...
var descending_order = [];

for( var i = (numbers.length-1); i >= 0; i--){
	descending_order.push(numbers[i]);
}
...
while( i < numbers.length ) { // i<5; sum=1+2+3+4+5=15
	sum += numbers[i];

	while( j < chars.length ) { // j<4; total=a+b+c+d=abcd
		total += chars[j];
		j++;
	}
...
//result =[abcd1, abcd3, abcd6, abcd10, abcd15]
